...
import { Loader2, Check, ArrowRight, FileSignature, Users, UserCheck, Phone, Handshake, Search, Save } from "lucide-react";
        ...
        <CardContent className="relative">
            <div className={cn(!selectedPelanggaran && "opacity-50 pointer-events-none")}>
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  {/* CATAT & LAPORKAN */}
                  <TabsContent value="catat" className="pt-4"><div className="p-4 border rounded-lg space-y-4"><div className="flex items-center gap-3"><UserCheck className="h-5 w-5 text-primary"/><h3 className="font-semibold">Catat & Eskalasi (Jika Perlu)</h3></div><p className="text-sm text-muted-foreground">Tandai kasus ini sebagai ditangani atau teruskan ke jenjang berikutnya jika kasusnya serius.</p><div className="flex items-start space-x-3"><Checkbox id="rekomendasi-bk" checked={formData.rekomendasiBK} onCheckedChange={v => handleFormDataChange('rekomendasiBK', v)} /><label htmlFor="rekomendasi-bk" className="text-sm font-medium leading-none">Sarankan Koordinasi ke Guru BK</label></div><div className="flex items-start space-x-3"><Checkbox id="rekomendasi-wakasek" checked={formData.rekomendasiWakasek} onCheckedChange={v => handleFormDataChange('rekomendasiWakasek', v)} /><label htmlFor="rekomendasi-wakasek" className="text-sm font-medium leading-none">Sarankan Koordinasi ke Wakasek Kesiswaan</label></div><div className="flex justify-end items-center pt-4 border-t gap-2"><Button variant="secondary" onClick={() => selectedPelanggaran && handleStatusChange(selectedPelanggaran.id, 'Ditindaklanjuti Wali Kelas')}>Tandai Selesai (Kasus Ringan)</Button><Button onClick={() => selectedPelanggaran && handleStatusChange(selectedPelanggaran.id, 'Diteruskan ke BK')}>Teruskan ke BK (Kasus Serius)</Button></div></div></TabsContent>

                  <div className="p-4 border-t mt-4 flex justify-end">
